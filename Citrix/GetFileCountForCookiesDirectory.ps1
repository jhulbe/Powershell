...
foreach ($profile in $profilenames){Write-host $profile (get-childitem \\server.cfins.com\user_ctx_profile$\$profile\UPM_Profile\AppData\Roaming\Microsoft\Windows\Cookies -ErrorAction SilentlyContinue).count}
